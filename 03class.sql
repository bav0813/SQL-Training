...
-- вывести количество менеджеров продаж и общее количество строк таблицы заказоа
 SELECT
	...
	COUNT('qqqqqtttt') AS count_all_3
FROM
	db_laba.dbo.orders;
...
FROM 
 	...
 	order by 1, 2;
 --select * from products where product_id = 137
...
SELECT
	...
	--oi.unit_price * oi.quantity - 1111,
	--*,
	oi.order_id
FROM
...
-- отсортировать по убыванию к-ва
 SELECT
	SUM(oi.unit_price * oi.quantity) as sum_total,
	--SUM(oi.quantity),
	oi.product_id
FROM
	db_laba.dbo.order_items oi
