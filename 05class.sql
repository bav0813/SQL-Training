...
where
	x.price_per_order > --79311.417503
	(
	SELECT
...
-- вывести номер заказа и его стоимость, имя и web сайт компании, имя и телефон продавца,
-- дату заказа и их стоимость
-- для заказов 2-го квартала 2017
-- для продаж выше среднего значения за весь период (по всей таблице)
 SELECT
	o.order_id,
