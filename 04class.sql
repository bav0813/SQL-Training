...
SELECT
	...
	--CAST(CAST((prod.list_price / prod.standard_cost -1)  * 100 as decimal(10,2) ) as varchar),
...
SELECT
	...
	CHARINDEX('ar', emp.last_name),	--oracle instr
	SUBSTRING ( emp.last_name , CHARINDEX('a', emp.last_name)+1 ,3 ),
...
join db_laba.dbo.product_categories pc on
	p.category_id = pc.category_id
	--p.list_price = pc.category_id -- err 
where pc.category_name != 'CPU' --pc.category_name not in ('CPU') <> 
order by 1;
--AMD Opteron 6378	651.92	CPU
/*
 * +--------+
 * |  LEFT  |
 * +--------+
...
--select * from db_laba.dbo.employees emp where job_title like 'sal%'
...
emp1.manager_id = emp2.employee_id
	--(emp1.manager_id = emp2.employee_id and emp1.dfdf = emp2.dsfsdfd)
...
where
	emp.job_title LIKE '%sal%' --or emp.employee_id <> 54
order by 1,2;
