...
-- сумму, среднее, к-во, мин и макс значения в разрезе атрибутов деталей заказа
 select
	...
	,MAX(o.quantity) OVER(PARTITION BY o.order_id ORDER BY o.item_id ) AS "Max2a"
from
	db_laba.dbo.order_items o
	where o.order_id in (1, 2)
group by o.order_id,o.item_id, o.quantity
...
-- SELECT YEAR(GETDATE()) -4, getdate(), YEAR(GETDATE())
 select
	x.order_id,
	x.item_id,
	x.quantity,
...
from
	...
	select
		...
		--SUM(o.unit_price * o.quantity)
	--			OVER(PARTITION BY o.order_id ) AS Total1 ,
...
select
	...
	NTILE(2) OVER (ORDER BY x.sold desc) AS Quartile2
from
	(
	select
...
-- для лимитов не болле 500
 select
	...
	ROW_NUMBER() OVER (PARTITION by credit_limit ORDER BY c.credit_limit, c.name) AS row_num2,
	RANK() OVER (ORDER BY c.credit_limit) AS Rank ,
	DENSE_RANK() OVER (ORDER BY c.credit_limit) AS DENSE_RANK,
	DENSE_RANK() OVER (ORDER BY c.credit_limit, c.name) AS DENSE_RANK2
from
...
-- для клиентов у который в имени 2я буква 'o' (латинская буква)
 SELECT
	...
	o2.price/
	LAG(o2.price)  OVER(PARTITION BY o.customer_id ORDER BY o.order_date, o.order_id) * 100 AS prevValProc
FROM
...
SELECT
	o.order_id,
	o.order_date,
	datediff(YEAR,o.order_date,	getdate()),
	datediff(day,o.order_date,	getdate())
from
	db_laba.dbo.orders o
