...
distinct p.category_id--(p.category_id)
  from db_laba.dbo.products p
  order by 1;
...
select p.product_name
 ...
 /*select distinct(tp.category_id), tp.product_namefrom db_laba.dbo.products tpselect tp.category_id, tp.product_namefrom db_laba.dbo.products tp
group by tp.category_id, tp.product_name
*/

 
-- вывести имя продукта, описание, категорию и стандартную стоимость
-- по продуктам 1-й категории
-- или со стандартной стоимостью более 5000
...
select p.product_name
  ...
  from db_laba.dbo.products p
 where p.standard_cost between 500 and 800
   and p.category_id NOT IN (1, 2, 3, 4/*, 999999*/) 
...
select p.product_name
	   ...
	   --,p.category_id
  from db_laba.dbo.products p
 where p.product_name like 'K%'--'k%'--'где третья буква K?   '__k%' '
 order by p.product_name desc;
